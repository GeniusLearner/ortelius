...
my @build_file_list = ( 'bin/dm', 'bin/libdmapi.so', 'bin/libhttp.so', 'bin/libiisplugin.so', 'bin/librestful.so',
			 'lib/dmtransfer', 'setodbc', 'lib/libtrilogycli.so', 'lib/libtrilogy.so');
...
my $build_dir = '/ombuild/751/linux-rocket/deployhub-oss/engine-64/DEBUG';
my $webadmin_build_dir = '/ombuild/751/linux-rocket/deployhub-oss/webadmin';
my $git_files_dir = '/opt/jenkins-slave1/workspace/DeployHub/DeployHub-OSS/installers/linux';
...
#  foreach my $pack_type ("rpm", "deb")
  foreach my $pack_type ("rpm")
  {
   ...
   my $pack_name = "deployhub-$component";
   ...
   {
     @deps = ( "deployhub-engine >= 8.0." . $build_number, "java-1.8.0-openjdk > 1.8.0.0");
   }
   ...
   if ($pack_type eq "rpm") 
   {
     ...
     {
      $fpm_cmd =  "fpm -s dir -t $pack_type -n $pack_name -v 8.0." . $build_number . "  --directories /opt/deployhub --rpm-user omreleng --rpm-group omreleng --rpm-defattrfile \"755\" --prefix /opt/deployhub --after-install $orig_cwd/engine/post-rproxy-install.sh $fpm_deps engine";
     }
     else
     {
